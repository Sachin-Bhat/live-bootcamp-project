...
pub async fn get_postgres_pool(url: &secrecy::SecretString) -> Result<PgPool, sqlx::Error> {
...
impl Application {
    pub async fn build(app_state: AppState, address: &str) -> Result<Self, Box<dyn StdError>> {
        dotenv().ok();
...
fn log_error_chain(e: &(dyn StdError + 'static)) {
    let separator =
        "\n-----------------------------------------------------------------------------------\n";
